...
int copyonwrite(pte_t *pte, pagetable_t pt, uint64 va) {
  char *mem;
  uint64 pa = PTE2PA(*pte);
  if (counter_get((void *)pa) <= 0) {
    panic("counter less than zero");
  }
  if (counter_get((void *)pa) > 1) {
    if ((mem = kalloc()) == 0) {
      return -1;
    }
